...
class LicenciaConsulta:
    """Clase para manejar la lógica de consulta y eliminación de licencias al servidor."""
    ...
    @staticmethod
    def eliminar_licencia(codigo):
        """
        Elimina una licencia comercial en el servidor.
        Retorna True si la eliminación es exitosa, False en caso contrario.
        """
        try:
            url = f"{LicenciaConsulta.BASE_URL}/eliminar/{codigo}"
...
class VentanaEliminarLicencia(tk.Toplevel):
    def __init__(self, master=None):
        ...
        self.btn_eliminar = tk.Button(self, text="Eliminar", command=self.eliminar_licencia, state="disabled")
        self.btn_eliminar.grid(row=3, column=1, padx=10, pady=10)
    ...
    def consultar_licencia(self):
        ...
        try:
            ...
            if licencia:
                # Habilitar campos para mostrar datos
                ...
                # Deshabilitar campos nuevamente (solo lectura)
                ...
                # Guardar el código para usarlo en la eliminación
    ...
    def eliminar_licencia(self):
        """Maneja la eliminación de una licencia en el servidor."""
        # Confirmar eliminación
        respuesta = messagebox.askyesno("Confirmar", f"¿Está seguro de que desea eliminar la licencia con código {self.codigo_licencia}?")
        if not respuesta:
            return

        try:
            exito = LicenciaConsulta.eliminar_licencia(self.codigo_licencia)
...
if __name__ == "__main__":
    ...
    ventana = VentanaEliminarLicencia(root)
    ventana.mainloop()